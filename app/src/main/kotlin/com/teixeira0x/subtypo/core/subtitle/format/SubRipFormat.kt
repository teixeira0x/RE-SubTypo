...
class SubRipFormat internal constructor() : SubtitleFormat("SubRip", ".srt") {
        ...
        for (i in cues.indices) {
            with(cues[i]) {
                sb.append(i + 1)
                    ...
                    .append(" --> ")
                    .append(endTime.getFormattedTime(timeFormat))
                    .append("\n")
                    .append(text)
                    .append("\n\n")
            }
            ...
            if (line.toIntOrNull() == null) {
                diagnostics.add(
                    Diagnostic(
                        ...
                        line = line,
                        lineNumber = index + 1,
                        message = "The expected is a number.",
                    )
                )
                index++
                continue
            ...
            if (index >= lines.size) {
                diagnostics.add(
                    Diagnostic(
                        kind = Diagnostic.Kind.ERROR,
                        lineNumber = index + 1,
                        message = "Missing timecode for: $cueNum.",
                    )
                )
                break
            }
